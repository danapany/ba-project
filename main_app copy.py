...
# main_app.py의 상단 import 부분을 다음과 같이 수정하세요
...
# main_app.py의 create_pdf_document_with_images 함수 개선
...
# visual_generator.py의 이미지 생성 함수들도 개선이 필요합니다
def generate_erd_improved(self, entities: List[Dict], relationships: List[Dict]) -> str:
    """개선된 ERD 다이어그램 생성"""
    # 한글 폰트 설정 확인
    ...
    with tab1:
        ...
        if uploaded_file is not None:
            ...
            with st.expander("📖 PDF 내용 미리보기"):
                # 수동 설정이 모두 입력된 경우
                if manual_endpoint and manual_key and manual_deployment:
                    manual_config = {
                        'endpoint': manual_endpoint,
                        'api_key': manual_key,
                        'deployment_name': manual_deployment,
                        'api_version': manual_api_version
                    }
                    generator = BAQuestionGenerator(manual_config)
                else:
        ...
        if st.button("🚀 문제 생성 시작", type="primary", disabled=not uploaded_file or not api_configured):
            if not api_configured:
                st.error("❌ Azure OpenAI가 설정되지 않았습니다. .env 파일을 확인하거나 수동으로 설정해주세요.")
